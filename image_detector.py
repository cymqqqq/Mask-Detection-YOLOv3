...
ap.add_argument("-i", "--image", required=True, help="path of input images")
...
ap.add_argument("-c", "--confidence", type=float, default=0, help="minimum probability to filter weak detections")
ap.add_argument("-t", "--threshold", type=float, default=0, help="threshold when applying non-maximum suppression")
ap.add_argument("-iou", "--intersection_union", type=float, default=0, help="threshold for intersection over union")
...
# py image_detector.py -i C:\Users\Acer\PycharmProjects\mask_detection_YOLOv3\test_images -o C:\Users\Acer\PycharmProjects\mask_detection_YOLOv3\mask_detection_v2\test_images_result  -w C:\Users\Acer\PycharmProjects\mask_detection_YOLOv3\mask_detection_v2\yolov3_last.weights -cfg C:\Users\Acer\PycharmProjects\mask_detection_YOLOv3\mask_detection_v2\darknet\cfg\yolov3.cfg -n C:\Users\Acer\PycharmProjects\mask_detection_YOLOv3\mask_detection_v2\darknet\data\obj.names
# py image_detector.py -i C:\Users\Acer\PycharmProjects\mask_detection_YOLOv3\test_images -o C:\Users\Acer\PycharmProjects\mask_detection_YOLOv3\mask_detection_less_dataset\test_images_result  -w C:\Users\Acer\PycharmProjects\mask_detection_YOLOv3\mask_detection_less_dataset\yolov3_last.weights -cfg C:\Users\Acer\PycharmProjects\mask_detection_YOLOv3\mask_detection_less_dataset\darknet\cfg\yolov3.cfg -n C:\Users\Acer\PycharmProjects\mask_detection_YOLOv3\mask_detection_less_dataset\darknet\data\obj.names
...
for image in os.listdir(args['image']):
    ...
    # display count
    text = "MaskCount: {}  NoMaskCount: {}".format(mask_count, nomask_count)
    cv2.putText(img, text, (0, int(border_size-25)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, border_text_color, thickness=2)
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            ...
            print(text)
            cv2.putText(img, text, (x, y-5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color=color, thickness=1)
            ...
            cv2.putText(img, text, (w-500, int(border_size-25)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, border_text_color, thickness=1)

    if IMAGE_PATH != '' and IMAGE_PATH is not None:
        cv2.imwrite(os.path.join(IMAGE_PATH, image), img)
